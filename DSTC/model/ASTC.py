...
class ASTC_model(nn.Module):
    ...
    def forward(
        ...
        x_16_compress = self.Down_compress_4(x_16)

        x_32 = x_16_compress.view(-1, 2, 4, 4)
        x_32 = x_32.permute(0, 2, 1, 3).contiguous().view(-1, 4, 8)
        x_32_compress = self.Down_compress_5(x_32)

        x_64 = x_32_compress.view(-1, 2, 5, 4)
        x_64 = x_64.permute(0, 2, 1, 3).contiguous().view(-1, 5, 8)
        x_64_compress = self.Down_compress_6(x_64)

        xhat_64, indices_64 = self.fsq(x_64_compress)
        xhat_32, indices_32 = self.fsq(x_32_compress)
        xhat_16, indices_16 = self.fsq(x_16_compress)
        xhat_8, indices_8 = self.fsq(x_8)

        xhat_64 = self.Up_compress_4(self.Up_compress_5(self.Up_compress_6(xhat_64)))
        xhat_32 = self.Up_compress_4(self.Up_compress_5(xhat_32))
        xhat_16 = self.Up_compress_4(xhat_16)
        ...
        xhat_64 = self.decoder_causal_conv(xhat_64)
        xhat_32 = self.decoder_causal_conv(xhat_32)
        xhat_16 = self.decoder_causal_conv(xhat_16)
        ...
        if return_recons_only:
            return x_recon_8, x_recon_16, x_recon_32, x_recon_64

        if return_discr_loss:
            real = orig_stationary_x
            fakes = [
                recon_stationary_x_8.detach().requires_grad_(True),
                recon_stationary_x_16.detach().requires_grad_(True),
                recon_stationary_x_32.detach().requires_grad_(True),
                recon_stationary_x_64.detach().requires_grad_(True)
            ]
            discr_losses = []
        ...
        recon_loss_64 = F.mse_loss(orig_stationary_x, recon_stationary_x_64)
        recon_loss_32 = F.mse_loss(orig_stationary_x, recon_stationary_x_32)
        recon_loss_16 = F.mse_loss(orig_stationary_x, recon_stationary_x_16)
        ...
        scaled_fakes = [recon_stationary_x_64,

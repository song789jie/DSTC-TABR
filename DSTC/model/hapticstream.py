...
class HapticStream(nn.Module):
    def __init__(
        ...
        self.causal_conv_end = nn.Conv1d(in_channels=64, out_channels=6, kernel_size=2, stride=1)
        self.encoder_causal_conv = nn.Sequential(
            ...
            *encoder_causal_conv_blocks,
            ...
            # self.causal_conv_end
        )
        self.causal_conv_begin = nn.ConvTranspose1d(in_channels=6, out_channels=64, kernel_size=2, stride=1)
        self.decoder_causal_conv = nn.Sequential(
            # self.causal_conv_begin,
            CausalConv1d(codebook_dim, layer_channels[-1], 3, pad_mode=pad_mode),
            *decoder_causal_conv_blocks,
    ...
    def forward(
        ...
        # print(tau.shape, (std_raw_x**2).shape)
        ...
        recon_loss = F.mse_loss(orig_stationary_x, recon_stationary_x)
        ...
        if return_loss_breakdown:
            return total_loss, (recon_loss, adversarial_loss, feature_loss, tau[0].item(), delta[0, 0].item())
...
if __name__ == '__main__':
    pass
    # test = HapticStream()(torch.randn((42, 1, 64)))
    # print(test.shape)
    # test_data = torch.randn(2, 1, 64)
    # print(torch.sqrt(torch.var(test_data, dim=-1, keepdim=True, unbiased=False)))
    # test = HapticStream().tau_learner(test_data, torch.sqrt(torch.var(test_data , dim=-1, keepdim=True, unbiased=False) + 1e-5))
    # print(test)
    # torchinfo.summary(HapticStream(), input_data=[torch.randn(42, 1, 64), False, False, True])
